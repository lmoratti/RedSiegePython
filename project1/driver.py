...
print(
    "WARNING: You are passing info via the commandline. An attacker with local access to your machine might be able to look at your bash history."
)
print(
    "In which case, they can see what IP addresses you've been looking at or your API key. OpSec is important, folks."
)


def queryIP(resource):
    r = requests.get(
        "https://api.ipgeolocation.io/ipgeo?apiKey=%s&ip=%s" % (apikey, resource)
    )
...
if isFile != "0" and isFile != "1":
    print(
        "The second argument should be a '0' if you are giving a single IP, '1' if a file with multiple IPs"
    )
